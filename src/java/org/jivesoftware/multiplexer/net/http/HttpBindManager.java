...
public final class HttpBindManager {
    ...
    public static final String HTTP_BIND_ENABLED = "xmpp.httpbind.enabled";

    public static final boolean HTTP_BIND_ENABLED_DEFAULT = true;

    public static final String HTTP_BIND_PORT = "xmpp.httpbind.port.plain";

    public static final int HTTP_BIND_PORT_DEFAULT = 7070;

    public static final String HTTP_BIND_SECURE_PORT = "xmpp.httpbind.port.secure";
        ...
        try {
            if (securePort > 0 && CertificateManager.isRSACertificate(SSLConfig.getKeyStore(), "*")) {
                if (!CertificateManager.isRSACertificate(SSLConfig.getKeyStore(),
                        ConnectionManager.getInstance().getServerName())) {
    ...
    public String getHttpBindUnsecureAddress() {
        return "http://" + ConnectionManager.getInstance().getServerName() + ":" +
                bindPort + "/http-bind/";
    }

    public String getHttpBindSecureAddress() {
        return "https://" + ConnectionManager.getInstance().getServerName() + ":" +
    ...
    public void setHttpBindEnabled(boolean isEnabled) {
        JiveGlobals.setXMLProperty(HTTP_BIND_ENABLED, String.valueOf(isEnabled));
    }
    ...
    public void setHttpBindPorts(int unsecurePort, int securePort) throws Exception {
        ...
        if (unsecurePort != HTTP_BIND_PORT_DEFAULT) {
            JiveGlobals.setXMLProperty(HTTP_BIND_PORT, String.valueOf(unsecurePort));
        }
        else {
            JiveGlobals.deleteXMLProperty(HTTP_BIND_PORT);
        }
        if (securePort != HTTP_BIND_SECURE_PORT_DEFAULT) {
            JiveGlobals.setXMLProperty(HTTP_BIND_SECURE_PORT, String.valueOf(securePort));
        }
        else {
            JiveGlobals.deleteXMLProperty(HTTP_BIND_SECURE_PORT);
        }
    }
    ...
    public void setScriptSyntaxEnabled(boolean isEnabled) {
        ...
        if(!isEnabled) {
            JiveGlobals.deleteXMLProperty(property);
        }
        else {
            JiveGlobals.setXMLProperty(property, String.valueOf(isEnabled));
        }
    }
